...
| 0.2 | **Per-turn cost cap** shows/enforces on screen | usage protection | ADR-0014 cap visible + enforced on the owner's screen | STATED — PROJECT.md:54 *"⬜ not yet proven on screen"*; [ADR-0014] | A? | ⬜ | `e2e/usage-protection.spec.js`, `e2e/nearest-limit.spec.js` — **re-run in isolation this session, 12/12 pass** (not part of the earlier interrupted full-suite run). Code path is solid; **owner-screen proof is categorically different and still ⬜** — I can confirm the code, not that you've seen it. |
| 0.3 | **Cross-turn cost rollover** | usage protection | ADR-0015 rollover carries across turns, visible | STATED — PROJECT.md:55 *"⬜ not yet proven on screen"*; [ADR-0015] | A? | ⬜ | `e2e/cost-rollover.spec.js` — **re-run in isolation this session, 3/3 pass**. Same distinction: code confirmed, owner-screen proof still ⬜. |
| 0.4 | **Durable per-chat cost across restart** | chat cost | ADR-0017 per-chat cost survives an app restart | STATED — PROJECT.md:56 *"⬜ not yet proven on screen"*; [ADR-0017] | A? | ⬜ | `unit/*`/`e2e/*` cost-store (not independently re-run this session — no dedicated spec was in the 4-file isolated run) |
| 0.5 | **Plain-language usage-limit message** | usage limit notice | ADR-0018 shows a human message when a cap is hit | STATED — PROJECT.md:57 *"⬜ not yet proven on screen"*; [ADR-0018] | A? | ⬜ | `e2e/payload-cap-notice.spec.js` — **re-run in isolation this session, 3/3 pass**. Code confirmed, owner-screen proof still ⬜. |
...
| 3 | **Attach / paste / drop files** | `#attach-btn` +, paste image, drag-drop onto `#composer` | attachment reaches the worker and influences its answer | **CONFIRMED, reproduced live**: attached a real text file, confirmed the chip rendered, sent, and captured the fake worker's actual stdin content via a throwaway instrumented copy (never touched the real fakebin) — both the file's content and filename reached the worker's message. Works correctly for text-file attach; image/paste/drag-drop use the same `attachments[]` path so are lower-risk but not separately click-tested. | **A** (text-file path) | Live reproduction, this session, 2026-07-27 | resolved for file-attach; a quick paste-image + drag-drop click is still worth doing on-screen |
| 4 | **Model selector** | `#model-select` (change) | the chosen model actually answers the next turn | **CONFIRMED, reproduced live**: selected "Opus 4.8" in the UI, captured the worker's real invocation argv (via the fake worker's `PCC_FAKE_CLAUDE_ARGV_FILE` hook) — `--model claude-opus-4-8` was passed, exact match. Works correctly. | **A** | Live reproduction, this session, 2026-07-27 | resolved — no owner action needed |
| 5 | **Correction chips (×9)** — "Check prior art", "Rabbit-hole check", "Push back", etc. | corrections bar buttons | each sends its exact canned instruction (alone or appended to the draft) | **CONFIRMED by direct source correlation** (renderer.js:124-134): all 9 labels and their sent messages are defined together in one static array (`{ label, msg }` pairs) and rendered/wired directly from it — they cannot desync without a code change touching this exact array. Read all 9; every label's `msg` honestly matches its stated intent. | **A** | Direct source read, this session, 2026-07-27 | resolved — no owner action needed |
...
| 7 | **Save the brief (Memory tab)** | `#memory-save` → `saveMemory` | edited PROJECT.md text is written atomically | **CONFIRMED, reproduced live**: edited, saved, survived a tab switch AND a full app restart. Test used the app's own `PCC_TEST_MODE` shadow-copy isolation (main.js:79-93) — verified the real `PROJECT.md` checksum was unchanged before/after. Works correctly. | **A** | Live reproduction, this session, 2026-07-27 | resolved — no owner action needed |
...
| S2.3 | **Close (X) fully terminates the app** | clicking the window close button kills the app and all underlying worker/child processes, permanently | **CONFIRMED, reproduced live**: started a real worker child process (long-running turn), closed the app mid-turn, checked OS process list before/during/after — the worker process count went 0 → 1 → 0. No orphan. `before-quit`/`window-all-closed` handlers (main.js:1969-1973) correctly call `killAllWorkers()`. One caveat: this used Playwright's `app.close()`, a close proxy, not a literal physical click on a real OS titlebar X — but it exercises the same Electron close event path. | **A** | Live reproduction, this session, 2026-07-27 | resolved — no owner action needed (the historical complaint appears fixed) |
