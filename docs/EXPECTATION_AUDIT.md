...
1. **CONFIRMED, live: the Send button silently stops working in a chat that isn't even busy.**
   ...
   reproduced end-to-end (Playwright, fake worker, zero real Claude usage). Switch to an idle chat
   while a *different* chat is mid-turn and try to send — nothing happens. No error, no queued
   delivery, the typed text just sits there. Root cause: `sendBtn.disabled = inRecovery || busy`
   (renderer.js:104) disables Send for **every chat**, not just the busy one. Waited 10s past the
...
|---|---|---|---|---|---|---|---|
| 0 | **Sending in one chat while another chat's turn is still running** | type + Send in an idle chat you switched to (per PR #83's fix, switching itself works) | the idle chat sends immediately — its own turn is independent of any other chat's | **CONFIRMED, reproduced live** (Playwright + the deterministic fake worker, zero real usage): switch to an idle chat while a different chat is mid-turn, type a message, click Send — **nothing happens.** The composer keeps the typed text; the log never leaves the welcome screen; no error, no bubble, no queued-later delivery. Root cause: `sendBtn.disabled = inRecovery \|\| busy` (renderer.js:104) disables Send **globally**, for every chat, whenever *any* chat has a turn running — not just the busy one. Waited a further 10s past the other chat's turn finishing; the stuck message never sent on its own — a real user would have to notice and manually re-click Send once the *other*, unrelated chat's turn happens to finish. Same global-`busy` gate also disables "New chat" (confirmed: silently no-ops, no new chat created — renderer.js:830), and (by the same code pattern, not yet click-tested) "Enable build session" (line 1167), the correction chips (line 1205), and "Second opinion" (line 1227). | **B — CONFIRMED** | Live reproduction, this session, 2026-07-27 (fake worker, no real Claude usage) + code read: renderer.js:104, 442-479, 830 | reproduced; still worth an owner click to feel the actual UX (button just sits grey, no explanation) |
