...
The normal verifier is Codex per `DECISION-023`. If Codex is unavailable and work should continue, `DECISION-033` allows a degraded fallback where Claude Code performs both roles. In that fallback, the same guardrail path still applies, but the verification result must explicitly disclose that it was self-verified and therefore did not receive an independent second-party review, using the standard wording in `docs/VERIFICATION_RESULT_SPEC.md` (`DECISION-036`).
