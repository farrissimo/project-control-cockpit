...
12. On a `PASS` verdict, close out the cycle with `scripts/close-out-verified-task.ps1` (`docs/HANDOFF_PACKET_SPEC.md`'s "Recommended Close-Out Order"), which archives, advances state, refreshes both live handoff artifacts, runs the post-close-out health check, and logs the event in one fixed sequence (`DECISION-034`). Committing the verified cycle remains a deliberate step (`-Commit`, or done manually); pushing to any remote always requires separate explicit owner approval.
