...
The normal verifier is Codex per `DECISION-023`. Per `DECISION-066`, Codex is available again as of this session, ending the `DECISION-033` fallback. `scripts/codex-verify-watcher.ps1` (`pcc-brr5-004`) is an available mechanism for this: a plain, non-AI polling script that watches `task-state.json` and invokes `codex exec` exactly once per task needing verification — never polling Codex itself, so idle time costs no Codex session usage, and real invocations happen at the same rate a manually-run Codex session already runs at (once per cycle). The owner starts (or schedules) this watcher deliberately; it is never started automatically by any other script. If Codex becomes unavailable again and work should continue, `DECISION-033` still permits the same degraded fallback where Claude Code performs both roles, with the same guardrail path and the standard self-verification disclosure (`docs/VERIFICATION_RESULT_SPEC.md`, `DECISION-036`).
