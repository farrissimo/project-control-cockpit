...
> - **I3 — CLOSED (durability ccd902e + concurrency [this slice]).** Durability half: lifecycle-state.json writes are atomic + retain `.prev`. Concurrency half: the two writers (main.js `setPhaseKind` + the spawned `lifecycle-advance.ps1` child) are both driven only from the single main process (single-instance app), and are now serialized through an in-process mutex (`app/state/mutex.js`), so neither can clobber the other's field — the interleaved lost-update is eliminated for every path that occurs in normal operation. **Documented residual (owner-accepted):** running `lifecycle-advance.ps1` by hand in a terminal *while the app is live* is not serialized against the app — an abnormal path (the pin is moved via the app); a cross-process lock is the noted upgrade if that ever becomes a real workflow. Tests: `mutex.test.js` proves the race is real (baseline) and that serialization eliminates it, incl. the concrete two-field lifecycle case.
