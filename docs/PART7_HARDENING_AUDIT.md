...
> **STATUS (2026-07-11, I1/I3 now closed for real):** an earlier "COMPLETE" claim was inaccurate (GPT secondary verification caught it) and was retracted; the owner then chose to close I1/I3 for real, and both are now done. **All 2 critical + 8 important findings are closed, and T3 is now closed too** (the backup push-failure path is tested). Only tech-debt T1/T2 remain (non-blocking, wording/docs only).
>
...
> - **I3 — CLOSED (durability ccd902e + concurrency e115d86 + project-binding [this slice]).** Durability half: lifecycle-state.json writes are atomic + retain `.prev`. Concurrency half: the two writers (main.js `setPhaseKind` + the spawned `lifecycle-advance.ps1` child) are both driven only from the single main process (single-instance app), and are serialized through an in-process mutex (`app/state/mutex.js`), so neither can clobber the other's field. Project-binding (GPT secondary-verification #4 caught this): serializing introduced a *new* race — the mutex could defer a queued op past a project switch (`projectDir` is a switchable global), so the op could mutate the wrong project. Fixed with `runExclusiveBound`: each op captures its project at request time and **fails closed** (`project_changed`) if the active project changed before it runs. Together these eliminate the lost-update / wrong-target for every path that occurs in normal operation. **Documented residual (owner-accepted):** running `lifecycle-advance.ps1` by hand in a terminal *while the app is live* is not serialized against the app — an abnormal path (the pin is moved via the app); a cross-process lock is the noted upgrade if that ever becomes a real workflow. Tests: `mutex.test.js` — baseline proves the race is real, serialization eliminates it (incl. the two-field lifecycle case), and the project-switch-during-queue op fails closed touching neither project.
> - **I1 — CLOSED (durability d40af30 + write-ahead journal [this slice]).** Durability half: task/project-state writes are atomic + retain `.prev`. Cross-file-consistency half: the paired write is now a JOURNALED TRANSACTION (`scripts/lib/state-journal.ps1`) — `Write-PairedStateAtomic` records both intended payloads in a write-ahead journal, applies both writes, then clears it; a kill at any point leaves a journal that `Resume-StateJournal` (run at the top of both writer scripts before they read state) replays to completion on the next run. So the two-file update is atomic-in-effect, robust BY CONSTRUCTION — it never guesses whether a split happened, it completes a recorded intent. (A field-comparison "detect the split" approach was tried first and REJECTED: no coupled field reliably diverges on every split, and project-state has its own independent update cadence — e.g. `updated_at` legitimately differs task-vs-project in the live repo — so comparison cannot distinguish a split from a normal separate update. Codex caught this before it shipped.) `validate-cockpit-state.ps1` now surfaces a lingering journal, so `doctor` reports an interrupted-but-not-yet-replayed update. **Documented residual (owner-accepted):** a journal from a crashed run persists until the next handback/advance run replays it (or `doctor` surfaces it) — no data is ever lost or left torn; the update simply completes on next run. Tests: `state-journal.spec.js` proves commit / crash-recovery / no-op / fail-closed-on-corrupt / doctor-visibility.
> - **T3 — CLOSED [this slice].** The backup PUSH-FAILURE path now has a test (`app/tests/e2e/sync.spec.js`): it forces a real offline push failure (origin → a nonexistent path) and asserts the handler is honest — `ok:false`, says "Push FAILED", and NEVER claims "backed up" (plus `ahead>=1`, `clean=false`). Codex-PASS (after a first-pass FAIL on second-app isolation, fixed by reusing the shared app as the last test). Only tech-debt T1/T2 remain — wording/docs only, no residual data risk.
> - The atomic-write primitive is now shared by JS (app/state/atomic-store.js) + PowerShell (scripts/lib/atomic-write.ps1, now also under the state-journal transaction). CI (.github/workflows/ci.yml) now also runs the node:test unit suite (96f0783).
...
- **T3 — Backup push-failure branch unexercised. → CLOSED.** `app/tests/e2e/sync.spec.js` drove only push-success; the `push.failed → ok:false` path had no test, so a regression could show "backed up" without pushing. Now tested: a dedicated broken-remote project forces a real offline push failure and asserts `ok:false` + "Push FAILED" + not "backed up".
