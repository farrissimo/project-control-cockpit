...
> **STATUS (2026-07-11, updated as I1/I3 are closed for real):** an earlier "COMPLETE" claim was inaccurate (GPT secondary verification caught it) and was retracted; the owner then chose to close I1/I3 for real. **I3 is now fully closed; I1's cross-file-consistency half is the remaining in-progress slice.**
>
...
>   - **I1 d40af30 — PARTIAL (durability done, cross-file consistency OPEN).** task/project-state writes are now atomic + retain `.prev`. NOT closed: a kill strictly between the two writes still leaves task advanced + project stale, and `validate-cockpit-state.ps1` is **not** a reliable backstop (it skips a still-null project verdict — validate-cockpit-state.ps1:58 — and does not run at all on the crash path). The earlier "validator backstop" wording was false and has been corrected in the scripts. **Next slice.**
...
>
> **Remaining to truly close I1** (in progress, owner chose to close it for real): make the task/project cross-file update either transactional or reliably detected+repaired on boot — the assurance-plan "cross-file consistency" requirement, which the atomic+`.prev` queue-item-4 prescription did not by itself satisfy. (I3's "revision/concurrency protection", class 9, is now satisfied by the in-process serialization above.)
