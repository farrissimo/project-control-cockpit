...
* brr-phase-2 (Babysitter Reduction Role Phase 2 - see DECISION-028)
* brr-phase-3 (Babysitter Reduction Role Phase 3, Verification Hardening - see DECISION-045)
...
`scripts/advance-cockpit-state.ps1` reads `.cockpit/result/verification-result.json` and applies this section's rules to `project-state.json` and `task-state.json` directly, instead of relying on manual reconciliation. A verified `PASS` maps directly to task status `complete`. The helper refuses to act if the verification result's `task_id` does not match the active `task-state.json` task, and it runs `scripts/validate-cockpit-state.ps1` after writing to confirm no drift was introduced. It then runs `scripts/refresh-live-handoff-artifacts.ps1` unconditionally, regenerating both `.cockpit/handoff/worker-directive.md` and `.cockpit/handoff/advisor-restart-brief.md` from the state it just wrote (`DECISION-035`) — a status change here changes `task_status` (and, on `PASS`, `last_verified_handoff`) out from under both live artifacts, and leaving either un-regenerated reproduces the exact staleness defect `pcc-brr2-001` and `pcc-brr2-004` each surfaced once.
