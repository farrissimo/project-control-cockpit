...
* Functionally tested (not read-through only): Branch-hygiene and file-structure checks tested directly against the real repo (a scratch branch-name value swap and a scratch .cockpit/ subdirectory rename, each reverted immediately) to confirm WARN/ISSUE behavior and that doctor.ps1 still exits 0 throughout. AMENDED 2026-07-04 (owner + Codex advisor agreed, after verification cycle 3, that the real repo cannot simultaneously be genuinely clean and carry this task's own in-progress edits): the Working-tree check's clean-state baseline and its induced dirty-tree case are accepted as satisfied via a disposable local git clone of this repo (git clone --local at current HEAD, with only the new scripts/doctor.ps1 copied in and committed inside that throwaway clone, never touching the real repo's history or any remote, deleted immediately after use) instead of literally against the real repo. This is the disclosed, owner-approved equivalent for this one structurally-conflicting criterion only; it does not relax functional-testing rigor elsewhere in this task or set a standing precedent for other tasks without their own separate owner review.
