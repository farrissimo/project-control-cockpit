...
- IDEA-014: Pre-Checkpoint Kernel Quality Audit
  Status: promoted-to-task
  Plain name: Pre-Checkpoint: Kernel Quality Check (DELIVERED)
  ...
  Notes: Owner-introduced 2026-07-04, explicitly scoped as pre-checkpoint work, not a Category A/B/C item itself. Deliberately narrow by design: findings are required to sort into exactly three buckets (real risks / maintainability smells / optional polish), with polish items ignored by default -- this is the guardrail against the audit becoming an open-ended rewrite pass, a taste contest, or an excuse to expand scope before the freeze. Promoted to active task pcc-checkpoint-001 on 2026-07-04, bundled with DECISION-074's own required extractability audit (one pass over the same kernel scripts answers both "is the contract clean" and "is the kernel good enough to freeze," per this entry's own "Relationship to the Maturity Checkpoint" section rather than two separate passes over the same files). DELIVERED via pcc-checkpoint-001 (DECISION-081): full report at docs/PRECHECKPOINT_KERNEL_AUDIT.md. Bottom line: extractability holds across all 23 scripts (one documentation gap, not a violation); one real, proven risk found (close-out-verified-task.ps1 / return-inadequate-work.ps1 near-duplication, already caused one bug fix to be applied twice by hand) left as a future optional task, not fixed here by design.
