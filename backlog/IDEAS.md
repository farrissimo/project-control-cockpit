...
1. IDEA-005 — Pre-task protected-file backup / restore point (only Tier-1 item that *structurally* cannot block completion)
2. IDEA-006 — `doctor` health-check, advisory / non-gating
3. IDEA-007 — `safe-stop` / clean-rollover command
4. IDEA-008 — Append-only failure/event log (non-blocking; enables honest metrics)
5. IDEA-003 — Runtime schema enforcement — **conditional: warn-by-default, not a hard halt**
6. IDEA-001 — Auto/CI-triggered enforcement — **defer: highest bloat + completion-blocking risk**
7. IDEA-009 — Deterministic retry governor — defer until PCC runs semi-autonomously
