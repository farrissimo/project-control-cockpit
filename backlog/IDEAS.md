...
- IDEA-020: One-click "Run integrity proofs" button (Verify view)
  Status: proposed (owner approved the idea 2026-07-12; build as its own slice AFTER Phase 5)
  Plain name: Assurance: Prove It Button
  Summary: A one-click button in the app's Verify view that runs the on-demand deterministic proofs (Phase 4 mutation proof + Phase 5 failure injection) and shows an honest pass/fail summary — no AI, no terminal.
  Details: none (full spec in Notes)
  Notes: Owner-approved 2026-07-12; use only if an idle window appears, else recorded here. Its own small slice (own test + Codex + gate); apply AFTER Phase 5 lands (the button runs BOTH proofs and scripts/run-failure-injection.ps1 must exist). Reuses the existing verify-product button pattern EXACTLY (renderer button → window.pcc.X() → main.js exec 'pwsh -File <script> -Json' → render a <pre>). Touch points: (a) main.js ipcMain.handle('pcc:runIntegrityProofs') execFile pwsh run-mutation-proof.ps1 -Json AND run-failure-injection.ps1 -Json (300s timeout, 8MB buffer); parse each receipt from stdout REGARDLESS of exit code (the scripts exit nonzero on SURVIVED/EXPOSED/INVALID — a real result, not a spawn error); resolve { mutation, failureInjection }. (b) preload.js: runIntegrityProofs invoke. (c) index.html #view-verify: a Run-integrity-proofs button + status span + result <pre>; add #run-proofs to the shared button style/:disabled rules. (d) renderer.js: click → disable, status "Running… ~1-2 min, no AI", render summary counts + per-case classification lines; honest coloring — GREEN only when killed>0 AND survived==0 AND exposed==0 AND invalid==0, else RED. No new dependency; all local/deterministic; receipts under .cockpit/evidence/ remain source of truth. Overlaps roadmap #21 (peek-under-the-hood).
