...
|------|---------------|-----------|---------|
| `claude-streamjson-success.txt` | `claude -p --input-format stream-json --output-format stream-json --verbose` (a real "reply PONG" turn) | crypto `signature` blob → `[redacted]`; ephemeral session/uuid/request ids → zeros; cwd genericized. Structure (system / rate_limit_event / thinking / assistant text / result lines) is otherwise verbatim. | `tests/unit/real-captures.spec.js` → `parseStreamJson` |
